...
export async function createHuntMap({ onSelect, onStatus }) {
  ...
  return {
    ...
    select(id,{pan=true}={}){highlight(id);const marker=markers.get(id);if(pan&&marker&&!map.getBounds().contains(marker.getLatLng()))map.panTo(marker.getLatLng(),{animate:false});},
    fit,
