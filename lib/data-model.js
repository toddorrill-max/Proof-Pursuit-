...
export function validateDataset(data, { mode = data?.mode, now = Date.now() } = {}) {
  ...
  for (const key of collections) (Array.isArray(data[key]) ? data[key] : []).forEach((row, i) => {
    ...
    for (const field of ['notes','phone','hours','barrelId']) if(row[field]!=null&&!text(row[field])) fail(`${p}.${field}`, 'must be nonempty text or null');
    if(row.phone!=null&&(typeof row.phone!=='string'||!/^\+?[\d(). -]+$/.test(row.phone)||row.phone.replace(/\D/g,'').length<7||row.phone.replace(/\D/g,'').length>15)) fail(`${p}.phone`, 'must be a telephone number without extension, or null');
