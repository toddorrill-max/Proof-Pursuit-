...
export async function loadDataset({ mode = dataConfig.mode, fetcher = globalThis.fetch, now = Date.now() } = {}) {
  if (!['demo', 'production'].includes(mode)) return { ok: false, data: null, errors: ['Unsupported data mode.'] };
  try {
    const response = await fetcher(new URL(`../data/${mode}.json`, import.meta.url));
