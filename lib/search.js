...
export function joinSightings(data, origin=null, now=Date.now(), {includeExpired=false}={}) {
  ...
  return data.sightings.filter(row=>includeExpired||row.status==='active').map(row=>{
