...
export function installChrome(active){
  ...
  if(nav){nav.replaceChildren();for(const [page,title] of [['index','Find bottles'],['store-picks','Store picks'],['rare-releases','Rare releases'],['retailers','Retailers'],['methodology','Verification'],['about','About']]){const a=link(title,route(page));if(page===active||active==='retailer'&&page==='retailers')a.setAttribute('aria-current','page');nav.append(a);}}
...
export function findingCard(row,{selected=false,ordinal=0,selectedId=null,onSelect=null,detail=false}={}){
  ...
  const heading=element(detail?'h2':'h3');heading.append(detail?document.createTextNode(row.pick?.name||row.bottle.name):link(row.pick?.name||row.bottle.name,route('finding',row.id)));article.append(heading);
