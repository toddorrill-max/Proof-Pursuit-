...
try{
 ...
 await page.goto(base+'/dashboard.html');await loaded(page);await page.locator('.hunt-marker').first().waitFor();
 ...
 for(const route of ['dashboard.html','retailers.html','retailer.html?id=demo-columbus','finding.html?id=sighting-0','store-picks.html','rare-releases.html','methodology.html','about.html']){
 ...
 await page.setViewportSize({width:320,height:900});await page.goto(base+'/dashboard.html');await loaded(page);await page.evaluate(()=>document.documentElement.style.fontSize='200%');
 ...
 const failed=await context.newPage();await failed.route('**/tile.openstreetmap.org/**',r=>r.abort());await failed.goto(base+'/dashboard.html');await loaded(failed);
 ...
 await production.goto(base+'/dashboard.html');await production.locator('#empty').waitFor();assert.equal(await production.locator('.finding').count(),0);assert.match(await production.locator('#demo-notice').innerText(),/not live inventory/);await production.close();
 ...
 await page.goto(base+'/dashboard.html');await loaded(page);assert.equal(await page.locator('#results img').count(),0);assert.equal(await page.evaluate(()=>window.injected),undefined);
 ...
 await measured.goto(base+'/dashboard.html');await loaded(measured);await measured.locator('.hunt-marker').first().waitFor();
