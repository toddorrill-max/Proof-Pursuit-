...
import { feedback } from './motion.js';
...
export async function createHuntMap({ onSelect, onStatus }) {
  ...
  const markers=new Map(),markerIds=new Map(); let currentRows=[],hadError=false,selectedId=null,started=false,rowsKey=null;
  ...
  return {
    setRows(rows,{refit=true}={}){
      currentRows=rows;
      const key=JSON.stringify(rows.map(r=>[r.id,r.point.latitude,r.point.longitude,r.releaseType,r.bottle.name,r.location.city,r.demo]));
      if(key===rowsKey){if(refit)fit();return;}rowsKey=key;
    ...
    select(id,{pan=true}={}){const changed=id!==selectedId;highlight(id);const marker=markers.get(id);if(changed&&pan)feedback(marker?.getElement(),{marker:true});if(pan&&marker&&!map.getBounds().contains(marker.getLatLng()))map.panTo(marker.getLatLng(),{animate:false});},
    fit,
